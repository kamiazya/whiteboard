...
- **complexity**: When a diff makes code harder to follow, it is answered by STRUCTURE — a kind-keyed table, a ranked list, one operation over an abstraction of two symmetric cases, a state union, a boundary between two jobs — not by an exemption in `biome.json` or helpers that move the same branches elsewhere. Measure with `node .claude/scripts/complexity-of.mjs --base origin/main --changed`, never by editing `biome.json`. `notApplicable` when no TypeScript logic changed
