...
- **dimensions**: defaults to `['correctness', 'contract', 'boundary',
  'test-coverage', 'reachability', 'background-work', 'complexity']`
  (`dead-code`, `auth` and `accessibility` are opt-in extras — pass them
  explicitly when relevant). The last three are default rather than opt-in
  for the same single reason: what they catch is CORRECT code. An unwired
  slice, a worker that runs on every instance, and a function that grew hard
  to follow all pass their tests and read as finished to every other
  dimension, so the only reliable catch is a lane that always asks.
  `complexity` measures with `node .claude/scripts/complexity-of.mjs --base
  origin/main --changed` rather than by editing `biome.json`.
