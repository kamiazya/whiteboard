...
- The model-facing side of a tool — its description, every parameter's `.describe()`, its annotations — is judged by ADR-0030's scoreboards, not by this skill: a new or reshaped tool re-pins its row in `tool-surface-quality.test.ts` and lands with every parameter described. Load `mcp-tool-surface` for the procedure.
