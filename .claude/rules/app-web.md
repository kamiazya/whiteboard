...
- `purity-guard.test.ts` (apps/web) and `file-size-budget.test.ts`
  (`tools/arch-lint`) hold files by explicit path on purpose and check the
  path still exists, so a move fails them loudly — the second in a project
  no `apps/web` command runs. `file-size-budget` is worse than a path list:
  it SCANS `apps/web/src` wholesale, so any web file crossing 800 lines
  fails somewhere a web run never looks. Telling readers to run it by hand
  was this rule's answer for a while, and a prose rung cannot notice being
  forgotten — the same push went out twice. **lefthook's pre-push block runs
  the whole `arch-lint-node` project**, which is where the guard lives now,
  so a web change is covered without remembering anything; run it by hand
  only to shorten the loop.
