...
**`ADAPTERS_REACHING_MECHANICS_CEILING` pins the count by equality — 39 today.**
...
carries the burn-down order; the four adapters it schedules first
(`restore.ts`, `live-doc.ts`, `workspace-document.ts`, `ws.ts`) hold 17 of
the 39.
