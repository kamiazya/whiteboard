...
| `packages/daemon-client` | the daemon's browser-safe client half: the `/api` Zod contracts the web app parses, the fetch/WS/SSE document backends, `api-client`, and the shared backend contract test suites. Extracted from mcp-server's `src/shared` so browser-safety is structural (this table scans it), not positional; mcp-server re-exports it through its published subpaths | model, server-core, zod, @opentelemetry (browser SDK set) |
