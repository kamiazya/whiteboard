...
- Run `pnpm test`.
- Resolve any `pnpm knip` finding one of three ways before closing: delete the dead code, drop the unused `export`, or register it in `knip.json` as an intentional public surface with a reason comment.
