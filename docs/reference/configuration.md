...
| `WHITEBOARD_BACKUP_DIR` | Where scheduled backups are written, and the setting that turns them ON. Unset means no scheduled backups — there is no destination worth guessing, since writing copies beside the data they protect is not a backup and anywhere else is a path only you know. Must be an ABSOLUTE path: a relative one resolves against the daemon's working directory, which is whatever your init system happened to pick, so `./backups` would mean something specific to you and land somewhere else. Each pass writes one timestamped directory (`2026-03-04T05-06-07.000Z`), containing exactly what `whiteboard server backup` produces — the same pass runs either way. Backups are taken WITHOUT stopping the server ([ADR-0021](../contributing/adr/0021-durability-boundary.md) decision 3). | unset (no scheduled backups) |
| `WHITEBOARD_BACKUP_INTERVAL_MS` | How often a scheduled backup runs. Must be a bare non-negative base-10 integer; a unit suffix or anything else **aborts startup** rather than silently becoming the default. Has no effect without `WHITEBOARD_BACKUP_DIR`, and setting it alone aborts startup too — a schedule with no destination does nothing while looking from the outside exactly like one that works. | `86400000` (24h) |
