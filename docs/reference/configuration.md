...
| `WHITEBOARD_BACKUP_TZ` | The IANA zone `WHITEBOARD_BACKUP_CRON` is read in — `Asia/Tokyo`, `Europe/Berlin`. Unset means the host's own zone, which in a container is usually **UTC**, so an unset value is why a carefully chosen 3am can turn out to be someone's afternoon. An unknown zone **aborts startup**. Has no effect without `WHITEBOARD_BACKUP_DIR`. | unset (the host's zone) |
