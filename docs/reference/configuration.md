...
| `WHITEBOARD_WORKSPACE_TAIL_MS` | How often this instance — a local daemon or a server-mode keeper — follows the stored record for each workspace that has a connected client (a WebSocket, or a live-sync event stream), so a browser attached to THIS instance receives what ANOTHER instance wrote. **Off unless set** — a single instance already learns about its own writes and would be polling for nothing. Set it only when more than one instance serves one record ([Running several instances](../how-to/self-host-with-docker.md#running-several-instances)). Must be a bare positive base-10 integer string. `0` and a blank value mean off — `0` is how you stop following without removing the variable — while anything else (`2s`, `1.5`, `-1`, `1e3`) **aborts startup**, so a mistyped value neither becomes an unintended interval nor silently disables following you asked for. Propagation is eventual by design ([ADR-0020](../contributing/adr/0020-coordination-boundary.md)): a client that misses a round converges on the next one, so this is a latency dial rather than a correctness one. A pass is not free, so pick the interval against what it costs: catching one workspace up is CPU-bound and holds the daemon for the duration — measured at 7.7ms for a small record, and 283ms for one with 300 commits of history that gained 50 — and a pass pays that once per subscribed workspace. A short interval over many workspaces spends most of the daemon on following. | unset (off) |
| `WHITEBOARD_REPLICA_TIER` | The read plane's default offline-replica tier (see [the security model](../explanation/security-model.md#replica-key-offline-read-plane)): `no-offline` (the daemon refuses to hand out a workspace's content key at all), `offline` (handed out with no expiry), or `bounded` (handed out with a `leaseExpiresAt`, which the browser — not this daemon — honours). Precedence: a per-workspace override set via `PUT /api/workspaces/:workspaceId/replica-tier` (`runtime:admin` bar) wins when present; otherwise this env var; otherwise `offline`. `PUT { tier: null }` clears the override back to that same fallback chain. Blank counts as unset; any other value **aborts startup**. | `offline` |
| `WHITEBOARD_REPLICA_LEASE_TTL_MS` | How long a `bounded`-tier lease lasts before the browser must discard the key it was handed. Must be a bare non-negative base-10 integer string, the same strictness as `WHITEBOARD_FILE_GC_INTERVAL_MS`; a present value that is anything else **aborts startup**. Has no effect unless the effective tier is `bounded`. | `604800000` (7 days) |
