...
| `WHITEBOARD_LEGACY_UI` | When set to `1`, the local daemon serves the retired `packages/mcp-server/src/app` UI (`dist/app`) at its root instead of the canonical `apps/web` build (`dist/web-app`). Escape hatch for the MCP-UI retirement (see [ADR-0001](../contributing/adr/0001-apps-web-canonical-frontend.md)); the legacy UI is removed once that migration completes. | unset (serves `apps/web`) |
