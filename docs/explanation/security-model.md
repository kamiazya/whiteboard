...
- **What `runtime:admin` is and is not.** It gates the daemon's process-state routes, so a credential without it cannot stop the daemon or delete its logs **over HTTP**. That is a real boundary for a caller that is not running as you — an OAuth client, or a paired web origin. It is **not** containment against a program running as your own user: `whiteboard daemon stop` does not call `/api/runtime/shutdown` at all, it reads the pid and sends `SIGTERM` (escalating to `SIGKILL` after 5s), and anything running as you can do the same by reading `ps`. No token can refuse a signal. Narrowing what a local agent holds is worth doing, but it does not make the daemon un-stoppable by that agent, and any copy claiming otherwise is wrong.
