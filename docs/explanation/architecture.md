...
| `wb_body_edit` | Replace passages of a markdown document's BODY. Each op quotes the passage it means and declares what that passage said when the edit was written, so a passage an intervening edit moved is still found and one an intervening edit changed is refused by name rather than overwritten — the addressing a line number cannot give. Either every passage applies or none does. |
