...
| Tool | Purpose |
|---|---|
| `wb_document_list` / `wb_document_resolve` | Find documents in a workspace. Creating, replacing and deleting them is `wb_workspace_edit` — there is no separate single-document tool for any of the three, so an agent reads one entry instead of four and the batch shape is what it reaches for by default. |
