...
| Tool | Purpose |
|---|---|
| `wb_document_list` | Find documents in a workspace; every row carries the id and the path, so an id is placed without a second tool. Creating, replacing and deleting them is `wb_workspace_edit` — there is no separate single-document tool for any of the three, so an agent reads one entry instead of four and the batch shape is what it reaches for by default. |
| `wb_workspace_edit` | Apply an ordered list of document operations — `document.create`, `document.set`, `document.delete` — to a workspace in one call, so seeding or reorganising a workspace does not cost one round trip per document. Ids minted by a `document.create` come back in the result. Unlike `wb_canvas_edit` this does **not** roll back: each document is its own CRDT, so a failure reports the op index that failed and how many before it stand, and the caller resumes from that index rather than resending the batch. One `actor` covers the whole batch — a batch has one producer, and it is recorded as OKF `generated.by` on the documents the batch writes; omitted, the writes are attributed to the server. |
...
| `wb_facet_set` | Tag up to 50 documents in one call (`tags.add` / `tags.remove` by name; every other tag stays; `tags.rename` renames or merges a tag throughout the documents named) — a markdown document's OKF core tags, a board's own tags, or with `nodeId` / `edgeId` one node's or one edge's, since a box and a relation are classified the way a note is (a scoped tag is `key:value`) — or set structured facet metadata on them, or — with `nodeId` or `edgeId` — on one node or one edge of one spatial document. The payload is shared across every document named, since "tag these as reviewed" is what a batch is for; two different payloads are two calls. Every document is checked to be in the workspace before any is written, so a bad id refuses the batch rather than tagging a prefix of it. `nodeId` or `edgeId` with several `documentIds` is refused: those ids are scoped to their own document, so the same one in two documents names two unrelated elements. A workspace's tag library (the document at `tags`) may restrict a key's values or make it one value at a time, and a tag outside it is refused before anything is written. |
| `wb_facet_list` | List the facets this deployment registered — the exact key to write, which objects each may be attached to, and the payload schema. With a `workspaceId`, also the workspace's own stencils, the tags it already uses counted by what carries each, and the keys its tag library declares (per key a description, exclusivity, and the admitted values with a colour each). |
...
| `wb_canvas_edit` | Apply a batch of edits to a spatial document in one transaction — add, patch, remove, lock and tidy nodes and edges; add or resolve comments (the annotation layer: a pinned remark anchored at a point or a node, drawn floating on every rendered surface — resolving is the only way to close one, for an agent and a person alike); plus `region.set` to make a group contain exactly the members named by id (the one op that deletes by omission; scoped by strict containment, so a node straddling the group's edge is never touched; a member that is elsewhere is moved in, and a new one is `node.add` with `within`). Where an op takes one id, a selector may go instead — `within` (every node inside a group) or `all` — so "lock everything" or "colour every box in this group" is one op that names the condition rather than a read to learn the ids. Either every op applies or none does, and a refusal names the op that failed by index. Node geometry is optional: a node with no `x`/`y`/`width`/`height` is placed below the existing content (inside a group when `node.add` names one with `within`, which grows to hold what is placed in it) and the chosen position is reported back under `geometry`. The result also carries the resulting board, so a caller never needs a second read to see what it just wrote. A batch of CONTENT changes is stored as a PROPOSAL for a person to adopt or dismiss rather than changing the document ([ADR-0029](../contributing/adr/0029-proposal-layer.md)) — that is the default, since nobody watches an agent type; `mode: "apply"` changes the document directly, which is what a surface a person is looking at passes. A batch carrying anything a proposal cannot represent — comments, locks, `tidy`, `region.set` — applies whatever the mode, those being claims and layout rather than content. `proposalId` keeps several calls in one proposal. A proposal carries node and edge adds, patches and removes — a verb with no single anchor (`tidy`) or one that deletes by omission (`region.set`) is refused by name, since neither could be adopted in part. |
...
| `wb_document_get` | Read a document's content. It takes `documentIds` and reads up to 20 in one call, each in the document's own format — OKF Markdown for a markdown document, JSON Canvas 1.0 (with the `x-whiteboard` extension) for a spatial one — and the format is not a parameter. Documents come back in the order asked for; one this call cannot read is named in `failed` with the reason rather than failing the batch, so a single pre-kind document does not cost the other nineteen. The ceiling is low because the content is UNTRUNCATED — surveying many documents is `wb_document_list` plus `wb_canvas_snapshot`, not this. Writing OKF Markdown back is `wb_workspace_edit`'s `document.set` op. |
| `wb_version_save` / `wb_version_restore` / `wb_version_list` | Save, list and restore versions in the document's history — the same history the History panel shows, so a checkpoint an agent takes is visible to the person watching, and a version a person saved can be rolled back to by an agent. `wb_version_save` takes `documentIds` and checkpoints up to 50 documents under ONE shared label — which is what makes them one checkpoint — answering a row per document as the panel lists it (`version.id` is what `wb_version_restore` takes). Every document's placement is resolved before any row is written, so a bad id records nothing; `wb_version_list` includes automatic checkpoints (`auto: true`). `wb_version_restore` reconciles the past state onto the document in place by default; `targetPath` restores it into another path instead (a new document, or an existing one with `overwrite: true`), and `subtree: true` rolls the document and every descendant back together. |
...
a `document.set` op on a spatial document and `wb_canvas_edit` on a
