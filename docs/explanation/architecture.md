...
| `wb_document_get` / `wb_document_set` | Read and replace a document's content. `wb_document_get` takes `documentIds` and reads up to 20 in one call, each in the document's own format — OKF Markdown for a markdown document, JSON Canvas 1.0 (with the `x-whiteboard` extension) for a spatial one — and the format is not a parameter. Documents come back in the order asked for; one this call cannot read is named in `failed` with the reason rather than failing the batch, so a single pre-kind document does not cost the other nineteen. The ceiling is low because the content is UNTRUNCATED — surveying many documents is `wb_document_list` plus `wb_canvas_snapshot`, not this. `wb_document_set` writes OKF Markdown. |
