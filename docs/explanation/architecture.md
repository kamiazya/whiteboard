...
| `wb_version_save` / `wb_version_restore` / `wb_version_list` | Save and restore labeled canvas versions kept inside the document itself. `wb_version_restore` reconciles the past state onto the document in place; it takes no target path. (Restoring into a different document is a capability of the daemon's file-backed version history, a separate system reachable today over its HTTP API only.) |
