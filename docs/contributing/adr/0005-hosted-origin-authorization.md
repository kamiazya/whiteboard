...
Chromium's LNA gating for **WebSocket** upgrades had not shipped when ADR-0002 measured this. It must be re-measured before the connection-ticket flow is finalised — if LNA starts gating the upgrade too, the ticket `fetch` and the socket open need to sit inside the same granted permission.
