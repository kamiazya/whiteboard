...
| [ADR-0039](0039-passkey-attestation.md) | A passkey attests a person's presence, not a device | Accepted — human gate 2026-09-16. Promotion attestation implemented end to end (#1610 daemon and contracts, #1612 browser and History, #1614 real-browser smoke); the user-verification gate (decision 6, refined 2026-09-17 to a per-workspace gate at the switch) and the profile (decision 9, to be ADR-0041) remain design. The browser stores no private key (the v5→v6 removal generalised); a row's human-reviewed claim comes from the OPERATION (`kind: 'human'`, not `auto`), and a WebAuthn assertion is evidence beside it, asked for where an agent forging the click would matter (promote always, proposal approval by setting, save never). Amends ADR-0035 decision 1's scope on the measured fact that a relying party cannot make a credential device-bound |
