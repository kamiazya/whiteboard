...
| [ADR-0039](0039-passkey-attestation.md) | A passkey attests a person's presence, not a device | Accepted — human gate 2026-09-16; design of record, nothing implemented. the browser stores no private key (the v5→v6 removal generalised); a row's human-reviewed claim comes from the OPERATION (`kind: 'human'`, not `auto`), and a WebAuthn assertion is evidence beside it, asked for where an agent forging the click would matter (promote always, proposal approval by setting, save never). Amends ADR-0035 decision 1's scope on the measured fact that a relying party cannot make a credential device-bound |
