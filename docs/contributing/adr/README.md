...
| [ADR-0040](0040-scoped-tags.md) | Scoped tags are the one classification vocabulary, on documents, boards, nodes and edges | Proposed — for the human gate 2026-09-16. `key:value` tags on markdown documents, spatial documents, nodes and edges (an infrastructure link is healthy or failing too; lines, being ink, get none); several values under one key allowed, with what search, the score (`multi`), the legend and the editor each do with them; `semantic.class/v0` retires unshipped; management as an in-use listing with rename/merge now and a declared library later. Supersedes ADR-0009 decision 3's spatial gap and ADR-0036 §6; gives ADR-0033 its `tags` reading |
