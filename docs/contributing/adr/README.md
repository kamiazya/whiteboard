...
| [ADR-0043](0043-authority-as-keys.md) | Authority is a key, delegation derives a narrower one, and revocation is declining to hand out the next | **Proposed** — design of record, nothing implemented. Replaces a withdrawn vocabulary-only draft that deliberately chose no mechanism (its own closing paragraph named that defect and it shipped anyway). TWO PLANES, not equally strong: the **act** plane is a token the daemon checks, so a bug in that check gets past it; the **read** plane is a content key, so arithmetic stops a holder whatever its disk holds — which is the only plane ADR-0042's "cryptographic rather than advisory" claim covers. Read authority attenuates by `HKDF(workspaceKey, documentId)`, act authority by a macaroon-style HMAC chain (caveats as Zod predicates, not Datalog), implemented natively over WebCrypto against libmacaroons' test vectors rather than imported. Biscuit is the named upgrade on two triggers — a verifier that must not be able to mint, or a third party in a caveat — and costs a WASM dependency plus a second policy language until then. UCAN and ZCAP-LD are rejected on a measured fact, not taste: both need the delegation's AUDIENCE to hold a signing key, and `browser-idb.ts`'s v5→v6 removal took that away deliberately. Two first applications named without an order — the agent's reach (a confused deputy: local-token ignores `requiredScopes`, a WS upgrade hands out `ALL_AUTH_SCOPES`) and the content-key tree |
