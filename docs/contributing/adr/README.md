...
| [ADR-0043](0043-capability-vocabulary.md) | Authority is held, attenuates one way, and never widens by being passed on | **Proposed — vocabulary and invariants only; no mechanism and no token format** (deliberately). Names what four existing places already do: `route-scope-registry.ts`'s scopes, pairing's origin+expiry token, ADR-0042's content key, ADR-0005's bootstrap token. Its one promoted rule is `daemon-token-only`'s own comment generalised — no route may be a path from a narrow credential to a wider one. Records that `createLocalTokenAuthStrategy` ignores `requiredScopes` by a STATED single-tenant concession, and the consequence that concession now also sizes what an AI agent holds (`ALL_AUTH_SCOPES`, `runtime:admin` included) — a confused deputy. Agent scoping is named as the first application and deferred to its own ADR; capability stays separate from ADR-0041's identity plane; revocation needing an online check is stated as a cost |
