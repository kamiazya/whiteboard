...
| [ADR-0042](0042-offline-revocation.md) | An offline policy decides whether a replica exists, not how long it lives | Accepted — human gate 2026-09-19; design of record, nothing implemented. An organisation states per workspace whether its workspaces may be replicated at all: `no-offline` (no replica, so revocation is immediate by construction), `offline` (the ADR-0023 default, revocation at reconnect), and `bounded` (a TTL lease) specified but NOT built — `security-model.md` records IndexedDB content as readable by whoever later owns the origin, so an expired lease is the app declining to render bytes still on disk; its trigger is ADR-0035's deferred E2EE. Reconnect applies the revocation and the purge is best-effort, the holding guarantee being that no FURTHER content reaches a revoked client. A revoked actor's offline edits are refused rather than merged (Loro carries no keeper-trusted clock, so "up to the revocation" is unanswerable) and kept for export rather than destroyed |
