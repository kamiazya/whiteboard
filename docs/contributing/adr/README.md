...
| [ADR-0035](0035-model-and-format.md) | The document model is native; JSON Canvas and OKF are projections | Accepted — human gate 2026-09-11; stands on the schema census in `packages/codec`, landing in dimension-sliced increments |
| [ADR-0036](0036-ocif-projection.md) | OCIF is a third projection, and its decompositions discipline the native model | Accepted — human gate 2026-09-12; stands on the OCIF ledger's measurement (native 15 against JSON Canvas's 21, and `degraded` 4 → 11, which is the format HAVING distinctions this model lacks). Splits relation from drawn line, and makes text a resource |
