...
| [ADR-0039](0039-passkey-attestation.md) | A passkey attests a person's presence, not a device | Proposed — the browser stores no private key (the v5→v6 removal generalised); a WebAuthn assertion backs a person at moments a person chose, and its absence is what marks an agent. Amends ADR-0035 decision 1's scope on the measured fact that a relying party cannot make a credential device-bound |
