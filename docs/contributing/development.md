...
```bash
pnpm test        # full suite (see root vitest.config.ts): mcp-node, mcp-smoke, model node, ports node, facet-engine node, codec node, loro-adapter node, search node, server-core node, arch-lint-node, canvas-render node/browser, canvas-viewer node/jsdom/browser, apps/web node/jsdom/browser (Playwright projects are slower)
...
For a fast, narrow pass while iterating on `packages/mcp-server` (selects only the `mcp-node` project out of the eighteen configured in root `vitest.config.ts`, so it also skips `mcp-smoke`, model node, ports node, facet-engine node, codec node, loro-adapter node, search node, server-core node, arch-lint-node, canvas-render node, canvas-viewer node/jsdom, apps/web node/jsdom, and all three browser projects (canvas-render-browser, canvas-viewer-browser, web-browser)):
