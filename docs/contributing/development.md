...
For a fast, narrow pass while iterating on `packages/mcp-server` (selects only the `mcp-node` project out of the eleven configured in root `vitest.config.ts`, so it also skips `mcp-smoke`, canvas-model node, canvas-render node, canvas-viewer node/jsdom, apps/web node/jsdom, and all three browser projects — not just the Playwright browser project):
