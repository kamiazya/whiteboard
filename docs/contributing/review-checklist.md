...
- [ ] Files stay under 800 lines — source and test alike, each held by its own shrink-only grandfather list in `tools/arch-lint/src/file-size-budget.test.ts`. Test files get the same budget rather than a higher one, because measured across this repo they run only 1.09-1.17x the size of source files at every percentile; the guard's own header carries the table. Functions stay under 50 lines, held the same way by `tools/arch-lint/src/function-size-budget.test.ts` beside it — a separate guard because a function's boundaries need an AST, and a separate CLASS of debt because a file budget is satisfied by moving mass between files while the long function survives whole.
