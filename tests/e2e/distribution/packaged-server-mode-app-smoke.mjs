...
try {
  ...
  // --- Scenario 5c: GET /api/workspaces/:wid/palette — workspace:read ---
  {
    const appEmpty = makeApp([])
    const res401 = await req(appEmpty, 'GET', '/api/workspaces/w1/palette')
    if (res401.status !== 401) fail('5c: expected 401 without auth', { status: res401.status })
    assertNoLeak('5c 401', await res401.text())

    const res403 = await req(appEmpty, 'GET', '/api/workspaces/w1/palette', { bearer: SMOKE_TOKEN })
    if (res403.status !== 403) {
      fail('5c: no workspace:read must be 403 on palette', { status: res403.status })
    }
    assertNoLeak('5c 403', await res403.text())

    const appRead = makeApp(['workspace:read'])
    const resPass = await req(appRead, 'GET', '/api/workspaces/w1/palette', { bearer: SMOKE_TOKEN })
    if (resPass.status === 401 || resPass.status === 403) {
      fail('5c: workspace:read must pass auth gate on palette', { status: resPass.status })
    }
  }
