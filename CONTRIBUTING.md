...
pnpm test         # all 27 vitest projects (listed under Workflow below)
pnpm typecheck
...
`pnpm test` runs 27 vitest projects. The names below are what `--project` accepts — worth copying rather than typing, because **vitest only errors when a `--project` filter set is empty**: a name that matches nothing alongside one that matches runs the smaller set and exits 0, which reads exactly like both suites passing.

| runtime | projects |
|---|---|
| node | `mcp-node`, `mcp-smoke`, `daemon-client-node`, `model-node`, `ports-node`, `facet-engine-node`, `plugin-visual-node`, `codec-node`, `arch-lint-node`, `loro-adapter-node`, `search-node`, `reference-graph-node`, `server-core-node`, `workspace-index-node`, `history-node`, `scene-node`, `canvas-render-node`, `canvas-viewer-node`, `web-node` |
