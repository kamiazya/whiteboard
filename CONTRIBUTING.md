...
- **pre-push** (the gate): runs seven checks in parallel — `pnpm verify:git-hooks`, `pnpm -r typecheck`, `pnpm lint:noconsole`, `pnpm lint`, `pnpm vitest run --project canvas-render-node packages/canvas-render/src/mutation-lane-coverage.test.ts`, `pnpm vitest run --project mcp-node packages/mcp-server/src/server/file-size-budget.test.ts`, and `pnpm vitest run --project arch-lint-node` — static gates plus two single-file guards and the architecture scans, so a broken build or a stray `console.*` in server code is caught before it leaves your machine. Test SUITES deliberately do not run here: the dev loop already ran the nearest layer for what changed, and CI runs everything on the push — running a suite a third time at push, on a machine saturated by the push itself, was double work and the origin of a load-dependent flake class. The two single-file guards are each here for something a suite cannot give. The mutation-lane one is the only check anywhere that notices the lane going stale: the lane covers a list of modules, so a new module is simply not covered and the weekly report still looks healthy. The file-size one is here for WHERE it lives — it scans `apps/web/src` from a test that sits in mcp-server, so running the web projects for a web change does not run it, and without this line the failure arrives a push later. Neither costs the gate anything: across two measured runs they took 11.0–15.1s and 7.7–10.7s, and in both the pre-push total was exactly `pnpm -r typecheck`'s own (68.9s, 75.3s). `arch-lint-node` is here for the same reason as the file-size one and on a wider scale: most of its files scan the repo from outside the package they govern (14 of 18 today), so running the project you changed never runs the guard that owns the rule you broke — a fixed sleep added to an mcp-server test leaves `--project mcp-node` green and fails this. It is a whole project rather than a single file because scoping it to those 13 would be a list to keep in step; it still runs no product code, opens no port and asserts on no clock, which is what the no-suite rule is actually about. Measured at 25.8s, 29.3s and 28.7s across three runs, with the total again exactly typecheck's own each time.
