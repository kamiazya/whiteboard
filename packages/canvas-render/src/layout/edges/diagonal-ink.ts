 ...
 * `r`, by open Liang–Barsky clipping — a chord riding a border or touching
 * a corner clips to nothing. Zero when the segment misses the rect.
 */
function insideFraction(a: Point, dx: number, dy: number, r: Rect): number {
  let t0 = 0
  let t1 = 1
  for (const [p, q] of [
    [-dx, a.x - r.x],
...
export function diagonalInkThrough(path: readonly Point[], rects: readonly Rect[]): number {
  ...
  for (let i = 1; i < path.length; i++) {
    ...
    for (const r of rects) {
      total += insideFraction(a, dx, dy, r) * length
    }
  }
  return total
}
