...
describe('toDocumentOutputPathErrorBody', () => {
  ...
  it('preserves err.code in body.error for each known code', () => {
    expect(
      toDocumentOutputPathErrorBody(new OutputPathError('output_exists', 'x'), 'ws1').body.error,
    ).toBe('output_exists')
    expect(
      toDocumentOutputPathErrorBody(new OutputPathError('invalid_output_path', 'x'), 'ws1').body
        .error,
