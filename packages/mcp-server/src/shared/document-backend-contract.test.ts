...
const jsPath = resolve(packageRoot, 'dist/shared/document-backend-contract.js')
const dtsPath = resolve(packageRoot, 'dist/shared/document-backend-contract.d.ts')
...
describe('browser-contract subpath export', () => {
  it('package.json exports map includes ./browser-contract', () => {
    ...
    expect(entry.import).toMatch(/document-backend-contract\.js$/)
    expect(entry.types).toMatch(/document-backend-contract\.(d\.)?ts$/)
  })

  it('dist/shared/document-backend-contract.{js,d.ts} exist', () => {
