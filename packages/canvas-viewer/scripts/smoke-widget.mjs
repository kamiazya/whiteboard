...
// Module scope because TWO scenarios serve the widget from it: the themed
// tool-result and the host-CSP run. A synthetic https origin that
// `page.route` fulfils, rather than file:// — measured, a cross-origin
// `fetch` from a file:// document never reaches Playwright's interceptor
// at all, so routing it there would count zero requests whatever the
// widget did.
const WIDGET_ORIGIN = 'https://widget.smoke.invalid'
// Read once; both font scenarios serve these same bytes.
...
/**
 * A plain render: no request at all, the bundled faces registered, and the
 * Japanese sample text actually drawn.
 */
async function checkPlainRender(browser, tmpHtmlPath) {
  ...
  if (hasRefreshControl) {
    fail('expected no Refresh control without an embedding host (file:// load has no parent frame)')
  }
  ...
  if (hasStickyNoteControl) {
    fail('expected no comment control without an embedding host (file:// load has no parent frame)')
  }
...
/**
 * The exception to zero-network: a themed tool-result fetches exactly one
 * face, from the catalogue origin and nowhere else.
 */
async function checkThemedToolResult(browser, injectedHtml) {
...
/**
 * A host's CSP, applied for real — the policy `_meta.ui.csp` declares and
 * nothing wider.
 */
async function checkUnderHostCsp(browser, injectedHtml) {
 ...
 */
async function checkSrcdocHosting(browser, injectedHtml) {
...
/**
 * Every scenario this smoke runs, in order.
 *
 * A TABLE rather than four calls in `main`, because the four were four
 * chances to forget one and the smoke cannot tell a skipped scenario from
 * a passing one — it is a single pass/fail, so fewer assertions is still
 * green. Measured before this: deleting the plain-render call left the run
 * printing PASS and exiting 0.
 *
 * This does not make dropping one impossible; it makes it a visible
 * deletion of a NAMED scenario rather than a missing call among four, and
 * the PASS line below reads back what actually ran.
 */
const SCENARIOS = {
  'zero network + fonts + JP text': (browser, ctx) => checkPlainRender(browser, ctx.tmpHtmlPath),
  'themed tool-result fetches one face': (browser, ctx) =>
    checkThemedToolResult(browser, ctx.injectedHtml),
  "under a host's declared CSP": (browser, ctx) => checkUnderHostCsp(browser, ctx.injectedHtml),
  'sandboxed srcdoc hosting': (browser, ctx) => checkSrcdocHosting(browser, ctx.injectedHtml),
}
...
async function main() {
  ...
  try {
    ...
    for (const [name, run] of Object.entries(SCENARIOS)) {
      await run(browser, { tmpHtmlPath, injectedHtml })
      ran.push(name)
    }

    if (process.exitCode !== 1) {
      // Names what RAN rather than what the script can do. Measured: with
      // the four scenarios called one by one, deleting a call left this
      // line printing its fixed summary and the run exiting 0 — a skipped
      // scenario was invisible. Reading them out of `SCENARIOS` is what
      // removes the individual call sites there was one to forget.
      console.log(`[widget-smoke] PASS: ${ran.join(', ')}`)
    }
  } finally {
