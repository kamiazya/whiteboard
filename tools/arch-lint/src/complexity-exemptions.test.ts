...
// cycle check and the mkdir lock, 7 -> 6 with the spatial editor, and 6 -> 3
// with the three MCP smoke scripts: the comment asks for the ceiling to
...
describe('the cognitive-complexity exemption list', () => {
  ...
  it('names only files that still exceed the threshold — a paid-down file leaves the list', () => {
    ...
    // hundreds, then 20 as the paydown passed 48, then 5. It moves again for
    // the same reason and by the same rule — to a number that still means
    // "not vacuous" rather than one the programme has to stop at. Lower it
    // again when it is in the way; never delete it, or the guard stops
    // measuring the moment the list empties.
    //
    // What is left is files a session MEASURED and chose not to split, so the
    // next move on any of them is a judgement rather than a sweep.
