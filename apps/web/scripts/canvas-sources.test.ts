...
import { endNodes } from '@kamiazya/whiteboard-model'
...
describe('docs/assets diagram sources', () => {
  it.each([
    ...
    //
    // `endNodes` rather than two field reads: a JSON Canvas file can
    // only author NODE ends, so every edge here names two — but the check is
    // "does what this names exist", and since ADR-0037 slice 3 an end may
