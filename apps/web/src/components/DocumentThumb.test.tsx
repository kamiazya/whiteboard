...
import { DocumentThumb } from './DocumentThumb.js'

afterEach(() => cleanup())

describe('DocumentThumb', () => {
  it('renders an img pointed at the latest-thumbnail route, encoding each path segment', () => {
    const { container } = render(<DocumentThumb workspaceId="ws-1" path="my canvas/x" />)
  ...
  it('swaps to the fallback icon and removes the img once loading fails', () => {
    const { container } = render(<DocumentThumb workspaceId="ws-1" path="a" />)
    const img = container.querySelector('img') as HTMLImageElement
    fireEvent.error(img)
  ...
  it('resets the failed state and re-renders the img when path changes on a reused instance', () => {
    const { container, rerender } = render(<DocumentThumb workspaceId="ws-1" path="canvas-a" />)
  ...
  it('applies the card wrapper classes and merges className for size="card"', () => {
    const { container } = render(
      <DocumentThumb workspaceId="ws-1" path="a" size="card" className="extra-class" />,
    )
  ...
  it('does not trigger a React setState-in-render warning when the guarded prevSrc reset runs on a src change', () => {
    ...
    try {
      const { rerender } = render(<DocumentThumb workspaceId="ws-1" path="canvas-a" />)
      // Changing path changes `src`, which drives the guarded prevSrc reset
      // (DocumentThumb.tsx L37-41) during this render — the React-sanctioned
    ...
    try {
      const { container, findByRole } = render(
        <DaemonApiContext.Provider value={daemonFetch}>
          <DocumentThumb workspaceId="ws-1" path="a" />
  ...
  it('revokes the previous object URL when the path identity changes', async () => {
    ...
    try {
      const { rerender, findByRole } = render(
        <DaemonApiContext.Provider value={daemonFetch}>
          <DocumentThumb workspaceId="ws-1" path="canvas-a" />
        </DaemonApiContext.Provider>,
      )
      await findByRole('presentation')
      rerender(
        <DaemonApiContext.Provider value={daemonFetch}>
          <DocumentThumb workspaceId="ws-1" path="canvas-b" />
        </DaemonApiContext.Provider>,
      )
