/**
 * The tier-1 editor's vessel: every facet its SUBJECT can carry, rendered
 * from the form the engine derives from each schema. The subject is a node
 * or an edge, and which facets it can carry is the registry's answer for
 * that subject's own contribution point — never a list kept here.
 *
 ...
 * so "a node and an edge at once" is not representable and the point below
 * follows from the tag rather than from a check that could disagree.
 */
export type FacetSubject =
  | { readonly kind: 'node'; readonly node: SpatialNode }
  | { readonly kind: 'edge'; readonly edge: CanvasEdge }

const SUBJECT_POINT: Record<FacetSubject['kind'], ContributionPoint> = {
  node: 'inspector.node',
  edge: 'inspector.edge',
}

const subjectOf = (subject: FacetSubject): SpatialNode | CanvasEdge =>
...
export interface FacetFormPanelProps {
  /**
   * The object whose stored values the panel SHOWS. For a node, writes go
   * to the whole selection (see `onWrite`) — the same split the
   * context-menu bands had, where the row reflected the node you opened on
   * and applied to every selected node.
   *
   * `undefined` when nothing is selected: the inspector says so and stays
   * open.
   */
  readonly subject: FacetSubject | undefined
  /**
...
export function FacetFormPanel({
  ...
  return (
    ...
    >
      {/* No dismiss control of its own. The panel is ABOUT the selection,
