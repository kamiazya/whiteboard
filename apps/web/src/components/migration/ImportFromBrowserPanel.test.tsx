...
describe('ImportFromBrowserPanel', () => {
  ...
  it('lists documents from the injected LocalStoreDouble', async () => {
    ...
    render(
      <ImportFromBrowserPanel
        workspaceId="ws1"
        daemonFetch={vi.fn()}
        browserStore={store.index}
        browserClock={store.clock}
  ...
  it('renders an empty state, disables import, and makes zero calls when there are no documents', async () => {
    ...
    await screen.findByText(/no documents kept in this browser/i)
    expect(screen.queryByRole('button', { name: /import/i })).toBeNull()
    expect(daemonFetch).not.toHaveBeenCalled()
    expect(settingsStore.load().migration.browserToDaemon?.lastImportedAt).toBeUndefined()
  })
  ...
  it('reports per-canvas success/failure and writes lastImportedAt only when >=1 succeeded', async () => {
    ...
    expect(settingsStore.load().migration.browserToDaemon?.lastImportedAt).toEqual(
      expect.any(String),
    )
  })
