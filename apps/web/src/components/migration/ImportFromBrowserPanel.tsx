...
import { importOneDocument } from './import-from-browser.js'

const log = getAppLogger('import-from-browser-panel')
...
interface ImportFromBrowserPanelProps {
  workspaceId: string
...
/**
 * Copy-first migration UI: lists documents kept in this browser and imports selected
 ...
 * Never writes to or deletes from browserStore/loroStore.
 */
export function ImportFromBrowserPanel({
  workspaceId,
  daemonFetch,
  ...
  useEffect(() => {
    let cancelled = false
    listLocalDocuments(browserStore, browserClock)
      .then((list) => {
      ...
      if (anySuccess) {
        ...
        settingsStore.update((current) => ({
          ...
          migration: {
            ...current.migration,
            browserToDaemon: {
              ...current.migration.browserToDaemon,
              lastImportedAt: now,
  ...
  if (documents.length === 0) {
    return (
      <p className="text-sm text-muted-foreground">
        No documents kept in this browser to import on this device.
      </p>
    )
  }
