...
describe('outlineFromScene', () => {
  ...
  // A run's own box is inside its block's, so keeping both would draw the
  // same ink twice and let a paragraph's words outvote the heading above it
  // when the projection caps by area.
  it('does not descend into a block’s runs', () => {
    const scene: Scene = {
      nodes: [
        {
          kind: 'paragraph',
    ...
    expect(outlineFromScene(scene)).toEqual([{ x: 0, y: 0, w: 400, h: 16 }])
  })
