...
// cache priming, no mocks), and clicking "Back to canvases" navigates away
...
describe('unknown-route recovery (real browser)', () => {
  it('loads the lazy not-found page and leaves it via Back to canvases', async () => {
    render(
      <MemoryRouter initialEntries={['/definitely/not/a/route']}>
        <App providerState={BROWSER_LOCAL_STATE} />
      </MemoryRouter>,
    )
    const back = await screen.findByRole('button', { name: /back to canvases/i })
